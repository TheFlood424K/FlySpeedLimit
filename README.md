...
A lightweight **Paper** plugin that provides an EssentialsX-style `/speed` command while enforcing configurable fly and walk speed caps — fixing EssentialsX’s broken `max-fly-speed` setting.
...
| [Spigot](https://www.spigotmc.org) | ⚠️ Partial | Functional, but Paper API extensions unavailable. Upgrade to Paper recommended. |
| [Folia](https://github.com/PaperMC/Folia) | ❌ Not supported | Region-threaded architecture is incompatible with the `PlayerMoveEvent` listener used for real-time enforcement. |
| CraftBukkit / Bukkit | ❌ Not supported | Missing API features required by this plugin. |

### Minecraft / Paper Versions

| Paper Version | Minecraft Version | Status | Notes |
|---|---|---|---|
| 26.1.2 | 26.1.2 | ✅ Primary target | Built against `26.1.2.build.66-stable`, Java 25 |
| 26.1.x | 26.1.x | ✅ Compatible | Same API surface |
| 1.21.4 | 1.21.4 | ⚠️ Legacy | Use v1.2.x of this plugin for 1.21.4 support |
| 1.21.x | 1.21.x | ⚠️ Legacy | Use v1.2.x of this plugin |
| 1.20.x and below | 1.20.x and below | ❌ Not supported | API differences may cause runtime failures |

> **Recommended:** Paper 26.1.2 with Java 25.
...
| Server software | Paper / Purpur / Pufferfish / Leaf 26.1.2+ |
...
> **Note:** Granting `flyspeedlimit.speed` implies both `.speed.fly` and `.speed.walk`. Grant each individually to restrict a rank to one command type.

### Per-Player Max Speed Overrides

Grant `flyspeedlimit.maxfly.<n>` or `flyspeedlimit.maxwalk.<n>` (where `<n>` is 1–10) to raise a player’s effective speed cap above the config default. The **highest** node the player has wins. If they have none, the config value is used.
...
| `flyspeedlimit.bypass` | Ignores all caps |
...
# Give "vip" rank both commands, fly cap 3
...
| `5` | 5× default |
| `10` | Minecraft hard cap |

---

## How the Fastplace Fix Works

Higher fly speeds cause the client to send more movement packets per second. Because Minecraft’s block placement is driven by client interaction packets rather than an independent server-side timer, faster fliers can place blocks more rapidly than players at default speed.
...
A player flying at 2× speed must wait 500 ms between placements, keeping their effective blocks-per-second identical to a player at baseline. Throttling only applies while the player is **actively flying**. Walking players and players with `flyspeedlimit.bypass` are unaffected.
...
The GitHub Actions workflow builds on every push to `main` and on every pull request. JARs are uploaded as workflow artifacts (30-day retention) and attached to tagged releases automatically.
